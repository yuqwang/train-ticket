...
public class AsyncTask {
    ...
    public Future<Response> checkSecurity(String accountId, HttpHeaders httpHeaders) throws InterruptedException{
        ...
        if(op < 1.0){
            AsyncTask.LOGGER.error("[Preserve Order Service] Delay Process，Wrong Preserve Process");
            Thread.sleep(4000);
        } else {
            AsyncTask.LOGGER.info("[Preserve Order Service] Normal Process，Normal Preserve Process");
        }

        AsyncTask.LOGGER.info("[Preserve Other Service][Check Security] Checking....");
    ...
    @Async("myAsync")
    public Future<Response<Contacts>> getContactsById(String contactsId, HttpHeaders httpHeaders) throws InterruptedException{
        AsyncTask.LOGGER.info("[Preserve Other Service][Get Contacts By Id] Getting....");
