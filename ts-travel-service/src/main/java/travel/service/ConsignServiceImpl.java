...
import travel.repository.ConsignRepository;
...
//        }
    ...
    @Override
    public Response queryByOrderId(String orderId, HttpHeaders headers) {
        List<ConsignRecord> consignRecordList = repository.findByOrderId(orderId);
        if (consignRecordList != null && !consignRecordList.isEmpty()) {
            return new Response<>(1, "Find consign by order id success", consignRecordList.get(0));
        } else {
